<div style="display:none;" class="author">
{
    "subtitle": "书籍pdf阅读优化方案",
    "title": "资源",
    "date" : "2025-07-13",
    "weather" : "sunny",
    "description": "书籍pdf阅读优化方案",
    "tag" : ["生活","感想"]
}
</div>



# 2025-07-13 第一版

## 大致思路
书籍pdf转成图片，导入扫描全能王优化清晰度。AI生成目录xml，使用WPS添加书签（会自动把书签识别为目录）。阅读时开启屏幕反色，就变成黑底白字了。

## 解决方案
### 高清优化
1. 下载书籍的pdf版本，推荐在zlib上搜索资源
2. 将pdf转为图片（自行调整dpi）
![](http://i0.hdslb.com/bfs/new_dyn/cfd1f85d03f903d2d5afed7418ce37923546948067920012.png)

3. 桌面版上传图片，等待同步
![](http://i0.hdslb.com/bfs/new_dyn/58caf3ab2561fd577db94e1bba6c0f7a3546948067920012.png)

4. 等上传完成，手机端进行高清修复
![](http://i0.hdslb.com/bfs/new_dyn/95d59dadd8cc09ec40d83c00bb1a6bcb3546948067920012.jpg)

5. 在网页端，下载pdf文件
![](http://i0.hdslb.com/bfs/new_dyn/aeb51a54749b431bf09e82cffb2adde63546948067920012.png)

注意：
反色后文字边缘噪点比较明显，看起来像是光晕一样，但并不明显。解决这个问题需要再次转图片，进行非局部均值去噪，很耗时间，就不弄了。
如果有水印，可自行添加裁剪功能。


### 目录生成

1. 上传目录截图，AI自动生成xml格式书签，推荐使用Kimi
![](http://i0.hdslb.com/bfs/new_dyn/86977588fe9210d2b6fb1f52cd8c95ae3546948067920012.png)

2. 对xml进行处理，添加参数（AI生成速度太慢），更新页码（因为目录的页码和PDF的页码不一致）
![](http://i0.hdslb.com/bfs/new_dyn/1285a870461242b317b584b97282b7863546948067920012.png)


3. 导入
![](http://i0.hdslb.com/bfs/new_dyn/eb9b3355859fab0c9fa96a066d6292a53546948067920012.png)




### 反色

1. 平板开启反色，效果图
![](http://i0.hdslb.com/bfs/new_dyn/08eb60430f11d15fd108bd97f3fe00b43546948067920012.jpg)

## 视频教程

<video src="http://sns-video-default.xhscdn.com/spectrum/1040g35831kjmvhk0ig105pgi0nb1om97bbou9i8" controls="controls" preload="metadata" video-id="0" ></video>

## 附件

### 书签更新
```python
import xml.etree.ElementTree as ET

def update_items(input_file,output_file,x):
        tree = ET.parse(input_file)
        root = tree.getroot()

        attributes = {
            "PAGE" : "1",
            "OPEN": "1",
            "FONTSTYLE": "0",
            "COLOR": "4278190080",
            "ZOOMMODE": "0",
            "PARMA": "0,0,0,0"
        }

        # 查找所有的ITEM元素并添加属性
        item_count = 0
        for item in root.iter("ITEM"):
            # 为每个ITEM添加属性，如果属性已存在会被覆盖
            for key, value in attributes.items():
                if key == "PAGE":
                    try:
                        value = str(int(item.get("PAGE")) + x)
                    except:
                        print(item.get("NAME"))
                item.set(key, value)
            item_count += 1

        tree.write(output_file, encoding="utf-8", xml_declaration=True)
        print(f"成功处理 {item_count} 个ITEM元素")
        print(f"处理后的文件已保存至: {output_file}")

if __name__ == "__main__":
    input_xml = "book/input.xml"
    output_file = r"output.xml"
    # x是偏移量，因为目录的页码和PDF的页码不一致
    x = 16

    update_items(input_xml,output_file,x)
```


### pdf2img
```python
import os
import time
from concurrent.futures import ProcessPoolExecutor

import fitz  # PyMuPDF
from functools import lru_cache
from PIL import Image

@lru_cache(maxsize=1)
def get_doc(pdf_path):
    return fitz.open(pdf_path)

def process_page(args):
    pdf_path, page_num, output_folder, dpi = args
    """处理单个页面并保存为图像"""
    doc = get_doc(pdf_path)
    page = doc.load_page(page_num)
    pix = page.get_pixmap(dpi=dpi)
    output_file = os.path.join(output_folder, f"{page_num + 1 :04d}.jpeg")

    img = Image.frombytes("RGB", (pix.width, pix.height), pix.samples)
    img.save(output_file, "jpeg")
    return "1"

def pdf2images(pdf_path, output_folder, dpi, workers):
    """使用多进程并行转换PDF页面为图像"""
    start_time = time.time()
    os.makedirs(output_folder, exist_ok=True)

    total_pages = len(fitz.open(pdf_path))

    tasks = [(pdf_path,page_num, output_folder, dpi) for page_num in range(total_pages)]

    with ProcessPoolExecutor(max_workers=workers) as executor:
        futures = [executor.submit(process_page, task) for task in tasks]
        for i, future in enumerate(futures):
            result = future.result()
            if result:
                print(f"Processed page {i + 1}/{total_pages}: {os.path.basename(result)}")

    print(f"Conversion completed in {time.time() - start_time:.2f} seconds")




if __name__ == "__main__":
    # 配置参数
    pdf_path = r"book/自己动手写cpu-雷思磊.pdf"
    output_dir = r"book/o"
    dpi = 300  # 根据实际需要调整
    workers = 20

    pdf2images(pdf_path, output_dir, dpi, workers)


```

### AI提示词

```prompt
技能:处理目录图片生成XML代码
1.接收用户提供的目录信息。
2.依据以下指定结构生成XML书签：
    -通过ITEM的嵌套和INDENT属性构建清晰的多级书签结构。
    -PAGE值需对应目录项在文档中的准确页码
    -NAME为准确的目录文本。
3.严格按照xml格式输出，不得出现其他无关内容。
===回复示例===

<?xml version="1.0" encoding="UTF-8"?>
<BOOKMARKS>
    <INFO PRODUCER="WPS PDF"/>
    <ITEM INDENT="1"  PAGE="18" NAME="1">
        <ITEM INDENT="2" PAGE="18" NAME="1.1">
            <ITEM INDENT="3" PAGE="18" NAME="1.1.1">
                <ITEM INDENT="4"  PAGE="18" NAME="1.1.1.1"/>
                <ITEM INDENT="4"  PAGE="18" NAME="1.1.1.2"/>
            </ITEM>
        </ITEM>
    </ITEM>
    <ITEM INDENT="1" PAGE="18" NAME="2">
        <ITEM INDENT="2" PAGE="18" NAME="2.1"/>
        <ITEM INDENT="2"  PAGE="18" NAME="2.2"/>
    </ITEM>
</BOOKMARKS>
===示例结束===
限制:
-输出必须严格遵循xml格式规范。
-仅围绕生成符合要求的XML书签相关内容进行回复，不回答其他无关问题。

```


# 2026-01-14 第二版

## 优化方案
使用扫描全能王对书籍图片高清化，虽然提升了整体清晰度，却带来了两个新问题：1.字体边缘残留的噪点像光晕，虽然不明显，但在一定程度上影响阅读体验 2.高分辨率图片占用空间过大，仅两张图片就需1MB存储，而降低分辨率又会导致质量下降，同样影响阅读体验。

为解决这个问题，可以将图片转换为黑白二值格式。这不仅大幅度节省存储空间，还消除了噪点，使文字呈现更加清晰。
这也引出了两个新问题。
1.书籍有些部分无法二值化，比如全彩插图等。解决方法是可以用原图替换，或者生成灰度图代替原图。
2.由于图片噪点影响，实际选择二值化的阈值还会存在一些问题。比如，如果阈值高，会导致文字过粗，阈值低，则文字会出现缺块的问题，甚至会同时存在文字过粗和文字缺块的现象，很难找到适合所有图片的阈值。解决方法是使用2bits灰度图。经过测试发现，2bits灰度图的效果对比1bit灰度图有质的提升，在细节上更丰富，阅读体验也更好。而3bits的灰度图已经几乎接近8bits灰度图的效果，对于有渐变效果的插图也有很好的显示效果。综合下来，可以使用2bits作为基底，用3bits作为补充。

实际操作中，开几个文件夹显示各类图片，手动选择并替换图片的过程非常繁琐，预览图片也不方便。因此我（实际上是AI）开发了一个辅助工具来简化流程，并在此基础上添加选择功能，选择区域进行替换，从而提升处理效率。

此外，随着修图大模型的发展，我发现使用千问的QImage大模型进行深度的修图，要远比普通的算法修图效果要好得多，所以可以先使用AI将原图修复成高清图，然后使用高清图转化成2bits,3bits的图片，这样生成的灰度图像会更清晰。当然这也可以用浏览器自动化工具来实现。需要注意的是，网页端的布局会改变，所以自动化工具也需要时常调整。


## 解决方案

### PDF转图片
按照第一版即可

### 图片处理
在扫描全能王中下载图片压缩包，并解压。
在小工具中导入图片文件夹路径，然后开始操作
![](http://i0.hdslb.com/bfs/new_dyn/ae84cfcb8c36b548b009a0cc0f734cf13546948067920012.png)
工具会开源，敬请期待

### 目录生成、反色
同第一版
